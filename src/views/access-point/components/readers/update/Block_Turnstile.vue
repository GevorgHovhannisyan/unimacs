  ...
  <div>
    <div class="vx-row">
      ...
      <div class="vx-col w-1/2 mt-3">
        <vs-input
          class="w-full"
          label="Hold Duration in seconds"
          @input="checkNumber($event, 'entry_exit_open_durations')"
          v-model="reader_data.entry_exit_open_durations"
        />
      </div>

      <div class="vx-col w-1/2 mt-6">
        <vs-checkbox class="mt-5 auto_lock" v-model="reader_data.door_sensor_autolock"
          >Door sensor autolock
        </vs-checkbox>
      </div>
    </div>
  </div>
...
export default {
  ...
  watch: {
    ...
    active_reader(data) {
      ...
      if (!this.active_point) {
        ...
        access_point = this.$store.state.accessPoint.edit_acu_data
          .new_access_points[this.active_point];
  ...
  computed: {
    ...
    isFloor() {
      ...
      let elevator_mode = this.$store.state.accessPoint.edit_acu_data
        .elevator_mode;
  ...
  methods: {
    checkNumber(value, field) {
      ...
      if (field === "impulse_time") {
        ...
        if (field === "entry_exit_open_durations" && value > 60) {
          this.reader_data[field] = 60;
        } else {
