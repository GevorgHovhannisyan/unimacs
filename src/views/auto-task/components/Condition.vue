<template>
  <div class="vx-row">
    <vs-popup
      class="holamundo condition_popup"
      title="Task"
      :active.sync="conditionPopup"
    >
      <div class="vx-row">
        ...
        <div
          class="vx-col w-1/2 mt-4"
          v-if="condition.condition_type === 'time'"
        >
        ...
        >
          ...
          <v-select
            v-model="condition.repeat"
            :options="repeat_options"
            placeholder="Repeat"
            @input="changeRepeat"
            class="select_padding w-full"
            :reduce="(repeat) => repeat.id"
          />
          <div class="week_days">
            <vs-checkbox
              class="condition_week_item mr-3"
              v-model="condition.DaysOfWeek[day.key]"
        ...
        <div
          class="vx-col w-full mt-4"
          v-if="condition.condition_type === 'direction'"
        >
          <v-select
            v-model="condition.EventsDirection"
            :options="direction_types"
            :reduce="(type) => type.id"
            placeholder="Event Direction"
            class="select_padding w-full"
          />
        </div>
        ...
        <div class="vx-col w-full condition_actions">
          ...
          <vs-button
            ...
            :disabled="!disabledConditionSubmit"
            @click="addCondition"
            >Submit</vs-button
          >
        </div>
    ...
    <div class="vx-col w-full">
      <vs-button class="mt-4 unimacs_button" @click="openCondPopup"
...
import Table from "./ConditionTable.vue";
...
time: 7000,
    ...
    addCondition() {
      ...
      this.$store.commit("autoTask/SET_CONDITION", { ...obj });
    },
    deleteCondition(event) {
      this.$store.commit("autoTask/DELETE_CONDITION", event);
  ...
  computed: {
    conditions() {
      return this.$store.state.autoTask.conditions;
    },
    event_list() {
      const events = [];
    ...
    disabledConditionSubmit(){
      const cond = this.condition
      if(cond.condition_type === "time") {
        return cond.TmBeginCondition && cond.TmBeginCondition 
      } else if(cond.condition_type === "event") {
        return cond.EventsCondition.length
      } else if(cond.condition_type === "direction") {
        return cond.EventsDirection !== null
      }
    }
  },
  watch: {
