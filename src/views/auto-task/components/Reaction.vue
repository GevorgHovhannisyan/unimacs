...
export default {
  ...
  created() {
      this.$root.$on('changed-task-level', () => {
          this.reactions.reaction_access_points = []
      })
  ...
  computed: {
     task_data() {
      return this.$store.state.autoTask.task_data;
    },
    command_list() {
      const commands = [];
  ...
  watch: {
    reactions: {
      handler: function (reaction_data, oldData) {
        this.$store.commit("autoTask/SET_REACTIONS_DATA", { ...reaction_data });
      },
      deep: true,
    },
  },
