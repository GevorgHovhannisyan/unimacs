            ...
            <div>
              ...
              <div class="vx-col w-full" v-if="credental.type == 'pinpass'">
                <vs-input class="w-full mt-4 pinpass" label="Input pinpass (4-8 digits)" type="text" name="pinpass"
                  v-validate="'validatePinnPass|min:4|max:8'" min="4" max="8" v-model="credental.pin_pass"
                  @input="checkPinn(credental.pin_pass)" />
...
export default {
  ...
  methods: {
    initTable() {
      const credentials =
        this.$store.state.cardholder.edit_cardholder_info.credentials;
      let arr = JSON.parse(JSON.stringify(credentials));
      if (arr.length) {
        arr.forEach((cr, index) => {
          if (cr.code) {
          ...
          if (cr.type) {
            cr.type_show = cr.type === 'vikey' ? 'web_pass' : cr.type;
          }
          if (cr.facility) {
    ...
    deleteCredential() {
      if (this.deletedItem.new_cr) {
        const credentalIndex = this.credentals.findIndex(
          (u) => u.code == this.deletedItem.code
        );
    ...
    //     this.credental.pin_pass = value.slice(0, 8);
    //   }
    // },
  },
  data() {
