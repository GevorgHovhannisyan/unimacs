<template>
  <div class="live_monitor_1_parent live_monitor_parents" :class="{
    ...
    <div class="live_monitor_1_parent live_monitor_parents">
      <div class="vx-col mt-3 cameras_dashboard" v-if="monitor_settings && monitor_settings.enable_cctv">
        <v-select v-model="cameraId" :options="access_point_cameras" class="select_padding" name="addresss" label="name"
          autocomplete="new-password" :reduce="(type) => type.id" :dir="$vs.rtl ? 'rtl' : 'ltr'" :searchable="false" />
      </div>
      <div class="xl:flex-row lg:flex-col live_monitor" :class="{
        ...
        <div v-if="monitor_settings.enable_cctv" class="w-2/3 video_block_dashboard" ref="video_block_dashboard"
          ...
          <div class="play_video">
            <LiveStream monitor="1" :cameraId="cameraId" @setCamera="cameraId = $event" v-if="cameraId" :dashboard="true"
              @fullscreen="fullScreenEnabled" class="dashboard_mon_1" />
          </div>
        </div>
        <VueDragResize v-if="monitor_settings &&
          ...
          :x="monitor_1_settings.miniature_drag_pos.left" :y="monitor_1_settings.miniature_drag_pos.top"
    ...
    horizontal: checkPosition('horizontal'),
      ...
      <ManualOverride :fieldsShown="fieldsShown" :monitor_1_settings="monitor_1_settings"
        v-if="monitor_1_settings && monitor_1_settings.access_point" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    // },
    getCameraName() {
      if (this.monitor_1_settings.access_point && this.access_points.length) {
        const find = this.access_points.find(
          (point) => point.id == this.monitor_1_settings.access_point
        );
        if (find) {
          return find.name;
        }
  ...
  watch: {
    monitor_1_settings(newVal, oldVal) {
      this.monitor_settings = JSON.parse(
        JSON.stringify(this.monitor_1_settings)
      );
  ...
  methods: {
    ...
    async resize({ left, top }) {
      ...
      settings.dashboard_settings.ap_monitor_1.miniature_drag_pos = {
        left: l,
        top: t,
      };
    ...
    fetchAccessPoints() {
      ...
      if (this.monitor_1_settings && this.monitor_1_settings.access_point) {
        const access_point = this.monitor_1_settings.access_point;
        this.$store
          ...
          .then((res) => {
            ...
            //  else if (camera) {
            //   console.log('if camera', res.data);
          ...
          .catch(async () => {
            ...
            if (
              ...
              if (
                ...
                settings.dashboard_settings.ap_monitor_1.access_point = null;
                await this.$store.dispatch("setSettings", settings)
    ...
    getCameraFromSettings() {
      ...
      const camera =
        settings &&
          settings.dashboard_settings &&
          settings.dashboard_settings.ap_monitor_1 &&
          settings.dashboard_settings.ap_monitor_1.camera
          ? settings.dashboard_settings.ap_monitor_1.camera
          : null;
    ...
    async fullScreenEnabled() {
      ...
      if (
        ...
        settings.dashboard_settings.ap_monitor_1.miniature_position = position;
        // setTimeout(() => {
        await this.$store.dispatch("setSettings", settings)
    ...
    getMiniatureSize() {
      const sett = this.monitor_1_settings;
      let height;
      let width;
