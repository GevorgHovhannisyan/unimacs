  ...
  <div class="clearfix register_page mt-4">
    ...
    <div class="flex items-center w-full">
      <vs-checkbox v-model="isTermsConditionAccepted" 
        > </vs-checkbox
      ><a download :href="getPdf('terms')" target="_blank" class="cursor-pointer">I accept the terms & conditions*</a>
      <vs-checkbox v-model="isPrivacyPolicyAccepted" 
      class="ml-10"
        ></vs-checkbox
      ><a download :href="getPdf('policy')" target="_blank" class="cursor-pointer " >I accept Privacy and Policy*</a>
...
export default {
  ...
  created() {
    ...
    this.$http
      .get(`registration/${this.$route.params.token}`)
      .then((res) => {
        if (res.data.length) {
  ...
  methods: {
    ...
    registerUserJWt() {
      ...
      this.$http
        .post(`registration/${this.$route.params.token}`, payload)
        .then((res) => {
          this.$vs.loading.close();
    ...
    sendMailAgain() {
      ...
      this.$http
        .put(`registration/${this.$route.params.token}`, payloadEmail)
        .then((res) => {
          this.$vs.notify({
time: 7000,
            color: "success",
