<template>
  <div>
    ...
    <vs-table noDataText="No data available" ref="multi_table" class="card_h_tbl bs_table"
      :pagination="card_list.length > itemsPerPage ? true : false" :max-items="itemsPerPage" search :data="card_list">
...
export default {
  ...
  methods: {
    setCards(event, value) {
      this.$store.commit("report/SET_CARDHOLDERS", { event, value });
    },
    setAll(event) {
      this.$store.commit("report/SET_ALL_CARDS", { event: event.target.checked });
    },
    filterByType(type) {
      if (type) {
  ...
  created() {
    if (!this.cardholders.length) {
      this.$store.dispatch("report/fetchCards").then((res) => {
        this.card_list = [...this.cardholders];
      });
    }
    ...
    this.$root.$on("clearReports", () => {
      this.$store.commit("report/SET_ALL_CARDS", false);
    });
  },
  computed: {
    headers() {
    ...
    cardholders() {
      return this.$store.state.report.cardholders;
    },
    selectAllIcon() {
    ...
    selectAllCheckBox: {
      ...
      set(value) {
        ...
        this.$store.commit("report/SET_ALL_CARDS", {
          event: value,
          type: this.group,
        });
    ...
    group: {
      get() {
        return this.$store.state.report.group;
      },
      set(value) {
        this.$store.commit("report/SET_GROUP", value);

        if (!value) {
          this.$store.commit("report/SET_CARD_SHOW", false);
    ...
    showSelected() {
      return this.$store.state.report.card_show_selected;
    },
  },
  watch: {
    show_selecteds(val) {
      this.$store.commit("report/SET_CARD_SHOW", val);
