      ...
      <vs-tab label="Main Settings">
        ...
        <div class="vx-row flexi_checkbox_block">
          <div class="vx-col md:w-1/4">
            <label for="schedule" class="vs-input--label">Repeat month</label>
            <v-select
              class="select_padding"
              name="schedule"
              v-model="editData.repeat_month"
              :options="repeat_months"
              placeholder="Repeat month"
              :reduce="(type) => type.id"
              :dir="$vs.rtl ? 'rtl' : 'ltr'"
            />
          </div>
          ...
          <div class="action_block vx-col md:w-1/4">
            <vs-button
              v-if="permissions.updateItem"
              :disabled="!this.editData.name"
...
export default {
    ...
    updateScheduleInfo() {
      if (this.editData.name) {
        const obj = {
          ...
          repeat_month: this.editData.repeat_month,
          // circle: this.editData.circle,
          // start_from: this.editData.start_from,
        };
        this.$parent.updScheduleInfo(obj, 'ordinal');
      }
    },
  ...
  created() {
    ...
    let repeat_month = [];
    for (let i = 1; i <= 12; i++) {
      repeat_month.push({ id: i, label: i });
    }

    this.repeat_months = repeat_month;
  },
};
</script>
